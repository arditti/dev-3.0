...
| return to live output from scrollback (terminal, touch) | **one round floating button** (`ScrollToLatestButton`, 48px, accent fill) bottom-right over the canvas, only while the pane is scrolled into history; tap = live tail, then it unmounts. Zero toolbar slots; the canvas keeps every other touch (`StatusDock`'s earned-not-permanent class). Its accent fill is the documented one-primary exception: it shows only while Send is off-target | an `ExtraKeyBar` key (permanent, off-screen at 390px), a fifth composer icon (the row sheds, never grows), a pane-bar button (pane navigation) | a viewport state earns a control only while it holds; compose mode swallows the canvas tap that ends copy-mode on desktop, leaving a phone no way back but swiping the whole history |
...
The app's secondary form factor is a **phone reached over `dev3 remote`** (any sub-768px viewport, phone or narrowed desktop browser). The desktop UI is dense, wide, and keyboard-first; it must **degrade to a touch-first, one-thing-at-a-time form** on narrow screens — without becoming a second app. This section is the canonical ruleset; the board carousel (Ittai Zeidman's idea) is the **reference implementation** the rest generalises from.
...
Rules: **gate layout on `useNarrowViewport`** (reactive, viewport-width) — unless the panel does not own the window, in which case measure its container (`useContainerWidth`) and treat width 0 as *not measured*, never *narrow*: the dashboard's spaces rail is gated this way, on the flex row holding both panels — measuring its own sibling would let showing the rail shrink the number that decides it. Use `useMobile()` for the `<meta viewport>` choice and the portrait-only device guard. Never gate a layout on `isElectrobun` (transport ≠ width) — browser mode can be wide, desktop can be narrowed. `useViewport()` serves **device-width** to the browser so a phone reports its true width and the media queries fire. The layout gate is **768 / `useNarrowViewport`**, never `useMobile()`.

**Portrait-only device guard — `Observed`:** the device class (not a narrowed desktop window) is locked to portrait where the Screen Orientation API allows; otherwise `MobilePortraitGate` blocks the shell in landscape with a rotate prompt and makes the app inert.
...
| Terminal panes | tiled tmux panes | **pane carousel** — one zoomed pane, axis-arbitrated horizontal swipe, a slim top bar, Arrow keys; keep-zoom via `tmuxPaneNavigate` (`MobilePaneCarousel.tsx`) | `Observed` |
| Terminal windows | tmux windows (workspaces) | **window switcher** — a slim ‹ prev · named dropdown · next › bar above the pane bar; buttons + Arrow-while-focused, no swipe (that is the pane carousel's). Only for window count > 1; `tmuxWindowNavigate` (`MobileWindowCarousel.tsx`) | `Observed` |
| Terminal text input (touch) | direct typing into the focused terminal | **docked composer** (gate = `!isElectrobun && isTouchDevice`, NOT width — an input-model switch): a terminal tap never summons the OSK; the composer above `ExtraKeyBar` owns text entry, and its sticky `⌨` **raw** toggle restores typing. `UX_DECISIONS.md` (2026-07-02) | `Observed` |
| Terminal scrollback return (touch) | click on the canvas leaves tmux copy-mode | **round floating button** (`ScrollToLatestButton`), only while scrolled into history; tap = live tail. §10 row | `Observed` |
...
Doctrine:
- **The breadcrumb spine stays the touch nav backbone**: logo→dashboard, project name→board, project chevron→switcher dropdown, back/forward. A long task title must not push them off-screen; the switcher gets a ≥44px target and a `right-0` fallback so its dropdown never clips.
...
`BottomSheet` is the narrow rendering for: context-menu→action-sheet, board filters, column-jump list, "Move to", the inspector actions sheet, the diff file picker, and action-style modals. It slides up, respects `env(safe-area-inset-bottom)`, traps and restores focus, and dismisses on backdrop tap / swipe-down / Esc. A pure React component — works identically in desktop and browser (no native dialog, per the no-native-dialogs rule). Do not scatter ad-hoc sheets.

Evidence: `BottomSheet.tsx` (`GlobalHeader` kebab, `ActivityOverview` actions, `MobilePaneCarousel` manage sheet); `prefers-reduced-motion` suppresses the slide (`bottom-sheet-panel`).
...
| Touch target | **≥ 44×44px** for standalone/primary controls and sheet rows; the coarse-pointer floor is 24×24 (WCAG 2.5.8) | bump primary controls per surface — a blanket 44px floor inflates inline chips and wrecks card layout |
...
**The ambient readout is a desktop-width slot, and it is capped at one.** On roomy widths the header carries exactly one ambient resource readout (currently memory headroom): a *status* earns its place by being seen without looking. On narrow it folds into the kebab sheet with `PreventSleepToggle` / `RateLimitIndicator` — a phone header has room for the breadcrumb and one kebab, and a number nobody acts on at a glance gives that room back first. In the sheet it keeps the 44px target and opens its breakdown as a BottomSheet, not a popover.
