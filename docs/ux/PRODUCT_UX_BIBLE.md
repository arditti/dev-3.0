...
| HTML Artifact | — (conditional Runtime-bar entry; rows in the archived task modal) | docked panel, or popup (`openArtifactsInPopup`) | task | view, fullscreen, navigate history, find in content (⌘F), download HTML/ZIP, open in web browser | `types.ts (SharedArtifact)`, `TaskArtifactViewer.tsx`, `show-artifact.ts` |
...
Destinations: `dashboard`, `project` (Kanban — the daily home), `task`, `project-terminal`, `settings`, `project-settings`, `changelog`, `stats` (Productivity Stats / Velocity Cockpit), `agent-traffic` (§5.9). Debug: `gauge-demo`, `viewport-lab`.
...
Show location only. Text click navigates; a segment's chevron opens a **switcher over that segment's own object** — project name → project switcher, task `#seq-N` badge → variant menu (`SiblingPopover`, chevron only with ≥2 live variants, so it is never a dead control). Switching is not a command; no commands in breadcrumbs. A space the user came through is a **plain-link** level between home and the chip (`dev-3.0 / AI / Nanochat`); the tail always carries the switcher, naming the space on a space board and the project below it. The task segment also carries the passive **native-backend marker** (§5.6) — identity, not an action.
...
| Agent traffic (destination) | Live map of recorded message attempts, either presentation (§5.9) | inspect tasks/messages, search, history window, delivery filter, pick presentation, open task, short message | agent launch/resume, task lifecycle action, inferred replies, own chrome above the header | `agent-traffic/AgentTrafficScreen.tsx` |
...
| Task artifact viewer | Interactive HTML plus local CSS/JS/raster assets via `dev3 show-artifact`; docked resizable panel by default, the image lightbox when opted in or slotless | sandboxed display, history nav, theme sync, floating ⌘F find over the content (§10), network integrations, HTML/ZIP download, open in the OS web browser (§10) | global destination, task lifecycle/git mutation, parent DOM/RPC access, native dialog (trusted scripts can self-navigate their iframe), a second viewer instance | `TaskArtifactViewer.tsx`, `utils/artifact-dock.ts`, `shared-artifacts.ts` |

**Archived tasks reach their agent outputs through enumerated rows, not the Runtime bar (`Observed`, 2026-08-08).** A completed/cancelled task has no Runtime bar, so `TaskDetailModal`'s archived view lists every image and artifact as its own clickable row (`SharedOutputsList.tsx`), above Notes: the deliverable outranks the commentary. Rows open the App-hosted viewers at their own index.
...
**An agent output is mounted by App; a pane only offers it a slot (`Observed`, 2026-09-07).** Images always use that App-hosted lightbox. One artifact viewer, three shapes: the **docked drag-resizable right panel by default**; that lightbox when `openArtifactsInPopup` is on or nothing publishes a slot (archived modal, immersive fullscreen, narrow, an off-screen toast); **hidden** when the route leaves its task, never a popup over the next board. It portals into the slot, re-hosted on a flip (`decisions/2026/09/07/artifact-panel-default-popup-opt-in.md`). A new task-scoped viewer inherits the lightbox; a second dock needs a record.
...
- **Inline comments in the markdown preview — selection-triggered:** the same action, reached the only way a rendered document allows. Select prose inside a file's md preview → a **floating `Add comment` button** over the selection (zero toolbar slots, same exemption class as the split-resize grip in §10) → the shared composer opens under the selection → the comment lands in the *same* per-file/per-side/per-line thread as a gutter comment, because rendered blocks carry their source line range (`data-md-line-start/end`, from mdast positions). Side follows the rich-diff block: `removed` comments `oldFile`, `added`/`context` comment `newFile`. **Rules:** no gutter, no per-block hover `+`, no "comment mode" toggle in the toolbar. A selection that resolves to no source line (mermaid SVG, an image, a caret-only click) shows **no** button rather than a disabled one. Preview mode must also **show the file's existing local threads**, both halves required: the commented **block is marked in place** (accent left border, via the line-stamping component override — never by mutating the DOM under React), and the threads render **in full under the preview**, ordered by line, each row scrolling to and flashing its block. Bubbles are *not* interleaved between blocks: the document is parsed as one unit, so splitting it would break reference links and footnote definitions. GitHub threads on the file get one slim `N review threads → Source diff` link (never silently dropped).
...
- **Per-comment actions (budget: 3, no growth):** `Edit` (`neutral`), `Delete` (`destructive`), `Send to agent` (`secondary`) — the same pattern as the GitHub thread action, pushing that single comment into the task terminal. Sending is per comment and **sticky**: the comment is marked `Sent`, persisted with the review, and leaves the export payload, so `Copy review` and the batch send cover **only unsent** comments; editing a sent comment clears the mark. Once the agent resolves a comment, `Send` becomes `Reopen` — still three controls. The batch send marks rather than wipes (the pre-2026-08-10 auto-clear destroyed reviews whose delivery was never proven).
...
- **The review lives on the task record, and the agent closes it.** Comments are `Task.review` entries (`src/shared/review.ts`), one RPC per change: the same list on the desktop, in a remote browser and in the agent's `dev3 review list`. Each carries a typed anchor — `diff-line` here, `artifact-element` from the artifact viewer — and the export card lists both, artifact entries last. No TTL: the agent resolves a comment (`dev3 review resolve <id> --reply "…"`), its reply renders under the bubble and `Send` becomes `Reopen`; the user clears with **Reset review** or Delete. Clipboard is transport, not store; leaving the surface discards nothing. An old `localStorage` review imports once.
- **Artifact comment mode shares this review — `Observed` (2026-09-16):** the artifact viewer's one header toggle turns the frame into a picking surface (click reports selector, text, nearest heading) and opens a 22rem aside with the *same* composer and bubbles (`src/mainview/review`). The frame draws the numbered pins itself; ids it cannot place after a republish show as `Not found in this version`, never dropped. One batch button, unsent only. Escape leaves comment mode first.
...
- **Two presentations, one feature.** Experiment 2 (Coordination, default) and 1 (orbit) share data/filters/selection. A toolbar radiogroup picks the mounted stage; no preference means 2.
- **Beta, off by default.** Settings → System → Advanced Experience gates every entry point and the tip; off leaves no trace.
- **Presence follows data; the badge follows unread.** Kebab persists; the desktop pill needs project traffic, read or not. Per-browser last-look; unread caps at `9+`.
- **A route, not an overlay.** Entry points reach `/app/agent-traffic`: standard header, Back/Forward; Escape/⇧⌘M step back. Ninth destination by exception, never a nav item (yaml `global_nav`). Entry picks the trailing hour, plays it once; nothing else replays.
- **Identity, not ownership.** Endpoints carry real identity, role, current runtime and overview. Seq is display only; historical nodes have no dead links. Stable grid; wires avoid cards, never imply parentage. Quiet/hibernated bands disclose a count; hibernated stays grey below the rest; completed/cancelled carry a stamp.
- **Coordination transport.** Below the graph: play, pause, steps, speed, event slider; Replay restarts, Live exits. Wire verdicts/counts follow the cursor. Replay reveals parked endpoints without waking. Follow: pair glide, idle overview; named five-wide project blocks. Manual/minimap pan stops Follow; Focus frames one. Lists on request; calendar days auto-load local history.
- **Attempts, not receipts.** Writer pushes and lazy fs observers cover mixed-version appends; no renderer poll, close at exit. Finite directional pulses mark recorded attempts only. In 2, violet drops show direction; failures stay red. Verdicts live in details. Neither means read/queued. Reduced motion stops flights/glides.
- **Inspect before navigating.** Nodes/messages open details; only the explicit task action navigates. One composer, selected task only, is the sole write: user-origin, never launches/resumes; finished says why, not a box; a stopped session warns; a refusal keeps text. Lists work without WebGL. Search, delivery filters and history windows stay on-screen.
- **History is recorded events; unrecorded state is unknown.** Status and column rebuild from movement records; runtime and PR checks stay current. An unrecorded past reads neutral, never today's. Show retention, oldest day, page limits. Preserve complete subjects (old rows use body head), spill pointers, delivery proof. No inferred replies, importance, ownership, merge order, narratives or progress; no interactive terminal, import/export or synthetic queue; one read-only TEXT snapshot per task, never a screenshot.

Evidence: `agent-traffic/`, `agent-traffic.ts`, `agent-message-log.ts`.
...
- **Settings screen** (Global / Project): Global Settings uses a left-nav master-detail layout with ten Settings categories, localized entry search, and immediate RPC/local persistence; Project Settings keeps its existing tabs; destructive removal stays behind confirmation.
...
| notification preference | Settings → **Notifications**: Focus Mode, watch default, sound, browser alerts, Web Push | Tasks/System, toast setup, top-level destination | one home answers when and how task events reach the user |
...
| project grouping (`Space`) | the **dashboard** only: a left rail listing `Everything` + each space + the computed `Home` (name and member count only — no activity dots), a collapsible group header per space whose rows are **indented under it with a hairline rule**, membership chips + a `Spaces…` action on the project row, and **every space action behind one `…`** — carried identically by the rail row and the group header, placed against the space name rather than the far side of the column: `Edit projects…`, `Move up`/`Move down`, `Rename`, `Delete`. Selecting a rail entry **filters the overview in place**. The rail owns `New space`, as the last entry of its space list (see the row below for order), and appears only while the dashboard's own column is ≥1024px — measured on the container, never the viewport. Below that width the same choice moves into a **BottomSheet** opened from a filter control beside the dashboard search field, and the selection **survives the resize** | a space route/screen, a space colour, a stored `Home`, membership changed by drag, a per-space entry in global nav, a reorder mode in the rail, an add-only `+` beside the space name, a rail that hides on a viewport media query, activity dots on a rail row, a filter silently cleared when its control goes off screen, an inert reorder cluster on a board that cannot be reordered, an `All projects` label on a row that lists spaces as well, a sub-44px space menu on touch | a space never became a screen: it rides the board route as a subject, and the dashboard rail filters in place. Drag stays order-only because with many-to-many memberships a drag cannot show which edge it removed. One menu, not a menu plus a `+`: an add-only control cannot answer "take this project out", which sent the user to the project's own row to undo something they did from the space. The rail carries the same menu because a filter you cannot rename from is only half a surface. The rail's own gate is its **container's** width: it shares the window with app chrome in the desktop shell and with nothing at all in a browser tab, so the viewport was measuring the wrong box (`useContainerWidth`; width 0 means *not measured*, never *narrow*). A rail row shows a name and a count and nothing else: the amber/blue dots' only legend lived in the centre column's group headers, and four numbers on a 180px row read as one clump — the headers keep the split, in words. Narrowing the window never changes what the list shows: the sheet keeps the filter reachable, so only a space that stopped existing clears it. A space carries its own **hide-on-camera** flag (`Space.sensitive`, toggled from the same `…`) on top of the member-derived one: the client's name is the secret even when no single project of theirs is marked, and it blurs in the rail, the filter sheet, the group header, the row chips and the picker. The group header's `…` is a bordered 44px control at the row's right edge below `md`, folding back to the inline glyph beside the name above it — the rail's copy stays a glyph, being desktop-only. Zero spaces ⇒ the dashboard renders its pre-Spaces flat list, rail absent |
...
| board entry from a project row | the row's **name** (accent + underline on row hover, chevron accent + nudge) plus one always-present **footer row** per project: `↗ Open board` and the counts it is not showing, including `N in To Do` from `getAllProjectTasks().todoCount` | a dedicated `Open board` icon in the row's action cluster, pointing the row's task rows at the board instead of the task | the board is the product's centre, and a dashboard that lists every waiting task turned it into a detour with no stated reason to take it. The footer answers "where is the rest of my work" with a number instead of an arrow; the old background-summary line carried the same counts and led nowhere. |
...
| find in content (`⌘F` search over what a viewer renders) | **Content toolbar exists** (diff viewer §5.3) → a toggle + inline box in that toolbar, beside the other content controls. **Full-bleed content, no content toolbar** (terminal canvas, artifact iframe) → the search UI is a **floating bar over the content**, top-right; `⌘F` is the primary trigger and must be registered in `keymap.ts`. A **modal** viewer claims `⌘F` unconditionally; a non-modal one (the docked artifact panel, the terminal) only while focus is inside it, which leaves the browser's native find intact everywhere else in remote mode. **One earned magnifier icon button** in the viewer header may toggle that bar (accent-tinted while open) — an explicit §11 button-creep exception, and a *trigger for content UI* owning no state of its own | a search input living in the header, a second entry point per viewer beyond that one icon, a global find destination, a permanent always-visible search field over content | why in UX_DECISIONS 2026-07-30; do not let unrelated controls inherit the icon exception |
...
| Task artifact viewer | docked panel (default) / popup | narrow **forces the popup**: a 360px-minimum panel is unusable on a phone; fullscreen unchanged | `Observed` |
...
- **Settings category** — one of the ten Global Settings navigation items: Appearance, Tasks & Board, Notifications, Keyboard, Terminal, Agents, Accounts, API models, Workspace, or System.
