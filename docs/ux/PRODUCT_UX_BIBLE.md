...
| project grouping (`Space`) | the **dashboard** only: a left rail listing `All projects` + each space + the computed `Home`, a collapsible group header per space, membership chips + a `Spaces…` action on the project row, and rename/delete behind one `…` on the header. Selecting a rail entry **filters the overview in place** | a space route/screen, a breadcrumb into a space, a merged space board, a space colour, a stored `Home`, membership changed by drag, a per-space entry in global nav | a space is a grouping, not a place — the moment it gains a route the next question is "where is this place's board", which is the decision #257 rejected. Filtering keeps one board per project and one screen. Drag stays order-only because with many-to-many memberships a drag cannot show which edge it removed. Zero spaces ⇒ the dashboard renders its pre-Spaces flat list, rail and panel absent |
| cross-project task list on the dashboard | the **existing `ActiveTasksSidebar` component**, mounted with no current project: scope locked to global, switcher hidden, `All / Needs you` presets toggling the `is:attention` token, and a per-space summary strip (`SpaceSummaryStrip`) — one chip per space with its needs-you/working split, toggling the `space:"…"` token | a second, dashboard-specific task panel; a copy of the sidebar's row/tier logic | "no new panel" is about duplicating the sidebar, not about where it may appear. One component ⇒ one behaviour, one set of bugs, and the presets reuse the search DSL instead of resurrecting the removed attention scope |
